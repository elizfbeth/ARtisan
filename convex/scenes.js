    ...
    handler: async (ctx, args) => {
        ...
        const updatedObjects = scene.objects.map((obj) => {
            if (obj.id === args.objectId) {
                return Object.assign(Object.assign({}, obj), { position: args.position || obj.position, rotation: args.rotation || obj.rotation, scale: args.scale || obj.scale });
            }
            return obj;
        });
        await ctx.db.patch(args.sceneId, {
...
export const updateStatus = mutation({
    args: {
        sceneId: v.id("scenes"),
        status: v.union(v.literal("uploading"), v.literal("analyzing"), v.literal("generating"), v.literal("ready"), v.literal("error")),
